...
export class FuseVerticalNavigationComponent implements OnChanges, OnInit, AfterViewInit, OnDestroy
{
    ...
    styleAsideActive: any;
    // eslint-disable-next-line @typescript-eslint/member-ordering
    classAsideActive: any;
    @ViewChild('asideActive', {static: false}) asideActive: ElementRef;
     ...
     * @param item
     */
    toggleAside(item: FuseNavigationItem, index: any): void
    {
        // Toggle
        ...
        setTimeout(() => {
            const top = 70 * index - 1 * index + 65;
            const content = this.asideActive.nativeElement.offsetHeight;
            const client = this._elementRef.nativeElement.clientHeight;
            const c = client - top;
            if(c >= content){
                this.styleAsideActive = {'top': top + 'px'};
            }
            this.classAsideActive = 'fuse-vertical-navigation-aside-wrapper';
      }, 0);
    }
